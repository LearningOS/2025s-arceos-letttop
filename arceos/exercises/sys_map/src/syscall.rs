...
fn handle_page_fault(vaddr: VirtAddr, mut access_flags: MappingFlags, is_user: bool) -> bool {
    ax_println!(
        "handle_page_fault: vaddr={:?}, flags={:?}, is_user={}",
        vaddr,
        access_flags,
        is_user
    );
...
fn sys_mmap(
    ...
    // find free space for mapping
    let Some(vaddr) = aspace.find_free_area(
        unsafe { VirtAddr::from_mut_ptr_of(addr) },
        length,
        VirtAddrRange::new(aspace.base(), aspace.end()),
    ) else {
        warn!("No free space for mapping");
    ...
    if let Err(e) = aspace.map_alloc(vaddr, length, map_flags, true) {
        warn!("Failed to allocate memory for mapping: {:?}", e);
        return -LinuxError::ENOMEM.code() as isize;
    }

    // Mapping Flags
    // 0x001: shared
    // 0x002: private, copy on write
    // 0x020: ANONYMOUS
    ...
    let file = match get_file_like(fd) {
        Ok(file) => file,
        Err(e) => {
            ax_println!("sys_mmap: get_file_like failed: {:?}", e);
    ...
    file.read(&mut buf)
        .map_err(|e| {
            ax_println!("sys_mmap: read file failed: {:?}", e);
            LinuxError::EIO
        })
        .unwrap();
    ...
    aspace
        .write(vaddr, &buf)
        .map_err(|e| {
            ax_println!("sys_mmap: write to address space failed: {:?}", e);
            LinuxError::ENOMEM
        })
        .unwrap();
